 ...
 */
public class DetailBuyPresenter {
    private IDetailBuy view;
    private ChungKhoan ck;
    private GiaoDich giaoDich;
    private BaoCaoTaiSanCk taiSanCk;

    public DetailBuyPresenter(IDetailBuy view) {
        this.view = view;
    }
        ...
        switch (loaiLenh) {
            ...
            case "MP":
                ...
                if ((timeBeginMP >= 0 && timeEndMP <= 0) || (timeBeginMP2 >= 0 && timeEndMP2 <= 0)) {
                    insertGiaoDich(tk, maCk, khoiLuong, loaiLenh, banMin);
                } else {
                    view.transactionFail();
                }
                ...
                call.enqueue(new Callback<RestObject>() {
                    ...
                    public void onResponse(Call<RestObject> call, Response<RestObject> response) {
                        if (response.body().getCheckConnect()) {
                            ...
                            tk.setTienDangVe(String.valueOf(Double.parseDouble(khoiLuong) * Double.parseDouble(gia)));
                            updateAccount(tk);
        ...
        subscription.add(ApiClientImp.getInstance().getTaiSanCK("get_baocao_ck", maTk, maCk)
                ...
                .subscribe(new Subscriber<RestObject>() {
                    ...
                    public void onNext(RestObject restObject) {
                        if (restObject.getBaoCaoTaiSanCk() == null) {
                        } else {
                            taiSanCk = restObject.getBaoCaoTaiSanCk();
                            taiSanCk.setKhoiLuong(String.valueOf(Integer.parseInt(taiSanCk.getKhoiLuong()) - Integer.parseInt(khoiLuong)));
